  ...
  return (
    <>
      ...
      {!sidebar && <span onMouseOver={() => setdropdown(true)} onMouseLeave={() => setdropdown(false)} className='fixed md:right-12 right-10 md:top-5 top-3.5 z-30 cursor-pointer'>
        {dropdown && <div className='absolute right-4 bg-gray-800 shadow-lg border md:top-7 top-5 rounded-md px-5 w-32 py-4'>
          <ul>
            <Link href={"/myaccount"}><a><li className='py-1 text-white text-sm font-bold hover:text-red-700'>My Account</li></a></Link>
            {usertype == "Merchant" && <Link href={"/admin"}><a><li className='py-1 text-white text-sm font-bold hover:text-red-700'>Dashboard</li></a></Link>}
            <Link href={"/orders"}><a><li className='py-1 text-white text-sm font-bold hover:text-red-700'>My Orders</li></a></Link>
            <li onClick={logout} className='py-1 text-white text-sm font-bold hover:text-red-700'>Logout</li>
          </ul>
        </div>}
        <span   >
          {user.value && <MdAccountCircle className='text-xl md:text-3xl mx-2 text-white' />}
        </span>

      </span>}

      <div className={`text-white flex flex-col md:flex-row md:justify-start justify-center items-center shadow-md sticky top-0 z-10 bg-bgcolor ${!sidebar && "overflow-hidden"}`}>

        <div className='mr-auto logo  md:mt-2 md:mx-2'>
          <Link href={"/"}><a><Image src='/logo.png' alt='lg' width={175} height={55} /></a></Link>
        </div>
        ...
        <div className='nav md:mb-0 mb-1.5'>
          <ul className='flex space-x-2 font-bold md:text-lg '>
            ...
            {/* { <span onMouseOver={() => setdropdown1(true)} onMouseLeave={() => setdropdown1(false)} className='fixed md:top-5 md:ml-0 top-14.5 ml-7 z-30 cursor-pointer'>
              ...
              <span   > 
               <li className='hover:text-red-700 md:mr-8'>Products</li>
              </span>
            </span>} */}
        ...
        <div className="flex cursor-pointer cart absolute right-0 top-3.5 md:top-5 mx-5 items-center">
          ...
          {!user.value && <Link href={"/login"}><a><button className=" text-white text-sm bg-red-500 border-0 mx-2 px-2 py-1.5 md:py-2 focus:outline-none hover:bg-blue-600 rounded ">Login</button></a></Link>}
        ...
        <div ref={ref} className={`z-10 w-72 h-screen sideCart overflow-y-scroll absolute top-0 bg-gray-800 px-8 py-10 transition-all ${sidebar ? "right-0" : "-right-96"}`}>
